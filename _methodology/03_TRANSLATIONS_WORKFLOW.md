...
`fr/index.html` and `en/index.html` must mirror each other:
...
|---|---|---|---|
| **Page text** | Headings, paragraphs, captions, callouts | `fr/index.html` and `en/index.html` | ✅ Translated |
| **Image text (inside the PNGs)** | Labels, headings, equations rendered into the images themselves | `/images/*.png` (no source files) | ❌ Still in French — see below |
| **Image transcriptions** | English text that should appear inside the EN versions of the images | `image_translation_pack_en.md` (root + `downloads/en/`) | Partially complete — covers A1–A7, B1–B5, C1–C7 (19 images). 9 images still untranscribed: 00, 01, 02, 03, D1–D5. |
...
1. Edit BOTH `fr/index.html` and `en/index.html` together. Keep section IDs and structure aligned.
2. If captions describe an image whose internal text changed, update the figcaption AND the corresponding entry in `image_translation_pack_en.md` (both copies — root and `downloads/en/`).
...
- `c:\Users\gabca\OneDrive\Bureau\MODEL_master\image_translation_pack_en.md` (project root, outside `site/`)
- `c:\Users\gabca\OneDrive\Bureau\MODEL_master\site\downloads\en\image_translation_pack_en.md` (inside the served downloads)

The site copy is what gets downloaded by visitors. The root copy is the working/editorial copy. Keep them identical. A future cleanup could symlink or remove duplication, but for now both must be edited together.
